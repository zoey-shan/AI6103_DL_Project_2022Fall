...
from utils.skin_data_loading import *
...
def train_net(net,
    ...
    val_transform = DoubleCompose([
        DoubleToTensor()
    ])

    # 1. Create dataset
    try:
        train_set = SkinDataset(skin_path, img_scale, transform=train_transform)
        val_set = ValSkinDataset(skin_path, img_scale, transform=val_transform)
...
def show_plot_epoch(train_loss, valid_loss):
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig('skin_Epoch_trainVal_loss_plot_allAug.png', bbox_inches='tight') 
...
def show_plot_lossBatch(all_train_loss, all_valid_loss):
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig('skin_Batch_trainVal_loss_plot_allAug.png', bbox_inches='tight')
